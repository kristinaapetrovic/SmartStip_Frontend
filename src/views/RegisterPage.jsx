...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const fetchFaculties = async () => {
      try {
        const res = await axiosClient.get(`universities/${universityId}?include=faculties`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== passwordConfirm || password.length < 8) {
      setError("Lozinka mora imati najmanje 8 karaktera i mora se poklapati sa potvrdom");
      return;
    }

    // Student-specific validacija
    if (!universityId || !facultyId || !indexNumber || !typeOfStudy || !yearOfStudy ||
        !averageGrade || !fieldOfStudy || !streetAddress || !phoneNumber || !locationId) {
        ...
        <form className="loginForm" onSubmit={handleSubmit}>
          <div className="formGrid">
            <Col className="formColumn" lg={6} md={12} sm={12}>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ime i prezime" />
              ...
              <input type="password" value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} placeholder="Potvrda lozinke" />
            </Col>
            ...
            <Col className="formColumn" lg={6} md={12} sm={12}>
              ...
              <select value={universityId} onChange={(e) => setUniversityId(e.target.value)}>
                ...
                {universities.map((u) => (
                  <option key={u.id} value={u.id}>{u.name ?? u.naziv ?? "—"}</option>
                ))}
              </select>
              ...
              <select value={facultyId} onChange={(e) => setFacultyId(e.target.value)}>
                ...
                {faculties.map((f) => (
                  <option key={f.id} value={f.id}>{f.name ?? f.naziv ?? "—"}</option>
                ))}
              </select>

              <label htmlFor="indexNumber">Broj indeksa</label>
              <input type="text" value={indexNumber} onChange={(e) => setIndexNumber(e.target.value)} placeholder="Broj indeksa" />
              ...
              <select value={typeOfStudy} onChange={(e) => setTypeOfStudy(e.target.value)}>
                ...
                {typesOfStudy.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>

              <label htmlFor="yearOfStudy">Godina studija</label>
              <input type="number" min={1} max={maxYearOfStudy} value={yearOfStudy} onChange={(e) => setYearOfStudy(Math.min(e.target.value, maxYearOfStudy))} />

              <label htmlFor="averageGrade">Prosek</label>
              <input type="number" step="0.01" min="5" max="10" value={averageGrade} onChange={(e) => setAverageGrade(e.target.value)} />
          ...
          <button type="submit" className="formButton">Registruj se</button>
